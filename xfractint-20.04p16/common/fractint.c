...
extern void wasm_start_main_loop(void);
#endif
...
#ifdef WASM_BUILD
   if (adapter < 0) adapter = 0;        /* WASM: default to videotable[0] */
#endif
...
#ifdef WASM_BUILD
   /* In WASM we cannot block.  Hand off to emscripten_set_main_loop
    * which calls wasm_main_loop_callback() on each animation frame.
    * wasm_start_main_loop() sets adapter=0, initialises the fractal,
    * and runs calcfrac() chunk-by-chunk. */
